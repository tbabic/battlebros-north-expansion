this.nem_roaming_beasts_contract <- this.inherit("scripts/contracts/barbarian_contract", {
	...
	function create()
	{
		...
		this.m.Type = "contract.nem_roaming_beasts";
	...
	function createStates()
	{
		...
		this.m.States.push({
			...
			function update()
			{
				...
				{
					if (this.Flags.get("IsHumans") || this.Flags.get("IsBarbarians"))
					{
			...
			function onTargetAttacked( _dest, _isPlayerAttacking )
			{
				...
				else if (this.Flags.get("IsBarbarians") && !this.Flags.get("IsAttackDialogTriggered"))
				{
					this.Flags.set("IsAttackDialogTriggered", true);
					this.Contract.m.IsPlayerAttacking = _isPlayerAttacking;
					this.Contract.setScreen("Humans");
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			ID = "Task",
			Title = "Negotiations",
			Text = "[img]gfx/ui/events/event_43.png[/img]{While you wait for %employer% to explain what he needs you for, you contemplate on how silent and eerie the whole settlement was when you first arrived. %employer% raises his voice %SPEECH_ON%This place is cursed by the gods and haunted by unearthly beasts! They come in the night with glowing red eyes and take lives at their whim. Most of our cattle is dead and I fear that once there are no more, we are next to be torn apart. The other day I sent some of our clansment out to find and kill the beasts but we haven\'t heard of them since.%SPEECH_OFF%He sighs deeply.%SPEECH_ON%Follow the tracks %direction% and hunt down and kill those creatures so that we can live without fear again! We will find some reward for you.%SPEECH_OFF% | %employer%\'s looking out his window when you find him. There\'s a horn full with mead in his hand - and there\'s nothing but silence outside. He turns to you, almost somber.%SPEECH_ON%When you came here, did you realize how quiet it was?%SPEECH_OFF%You remark that you did, but you\'re a warrior and folk often become quiet in your presence. %employer% nods and takes a drink.%SPEECH_ON%Ah, of course. Unfortunately, it isn\'t that people are afraid of you. Not this time. We have had people being attacked these past weeks. Beasts of some sort are on the loose, we know not what they are, but only who they take. We\'ve made sacrifices to gods, but no help has arrived, except for your crew...%SPEECH_OFF%His next drink is a long one. When he finishes, he turns to you, empty cup in hand.%SPEECH_ON%Would you go hunt these monsters down? I don\'t like asking for help, but we need it. Of course, there will be a reward for you.%SPEECH_OFF% | %employer%\'s got a few dogs at his feet, all tuckered out with their tongues lolling.%SPEECH_ON%They\'ve spent the past few days hunting for missing folks. Folks that are seemingly disappearing to the gods know where.%SPEECH_OFF%He leans down and pets one of the hounds, scratching it behind the ear. Usually, a dog would respond to that, but the poor thing barely even responds.%SPEECH_ON%The folks don\'t know what I know, though, which is that people ain\'t just disappearing... they\'re being taken. Horrible beasts are afoot, warrior, and I need you to go after them. Hell, maybe you\'ll even find someone or two of the clansfolk, though I doubt it.%SPEECH_OFF%One of the mutts lets out a long, tired wheeze almost as if on cue. | %employer% welcomes you to his jurt, but there is a worried expression on his face. %SPEECH_ON% Have you heard of %randombarbarian%?%SPEECH_OFF%You nod your head, you know him as a good warrior and a man of honor. %employer% continues.%SPEECH_ON%A week ago we sent him and some clansmen %direction% of here to go hunt down some foul beasts that have been terrorizing us for weeks. They have not returned.%SPEECH_OFF%%employer_short% nods and sets the satchel down. He sits on the edge of his table.%SPEECH_ON%Why do you think that is? I think it\'s because he\'s dead, but let\'s not be so negative. I think it\'s because the beasts out there require more. I think they require someone like you, warrior. Would you be willing to help us now where my men failed?%SPEECH_OFF%}",
			Image = "",
			List = [],
			ShowEmployer = true,
			ShowDifficulty = true,
		...
		this.m.Screens.push({
			ID = "CollectingProof",
			Title = "After the battle...",
			Text = "[img]gfx/ui/events/event_22.png[/img]{Your men take the fools\' disguises lest your employer, %employer%, not believe your doings here. | Your employer might not believe what was going on here. You order your men to collect the disguises. %bro1%, stripping a mask off one of the slain, starts to wonder.%SPEECH_ON%So they dressed themselves up as the sort of thing to attract us, and now they\'re all dead. I hope they didn\'t think it a game.%SPEECH_OFF%%bro2% cleans his blade in the folds of one of the disguises.%SPEECH_ON%Well, if it were a game, I sure enjoyed playing it.%SPEECH_OFF% | %randombrother% nods at the slain.%SPEECH_ON%It\'s mighty likely that %employer% wouldn\'t believe a group of brigands were dressing up as beasts.%SPEECH_OFF%Agreeing, you order the men to begin collecting the masks and disguises as evidence. | You\'ll need evidence to show your employer, %employer%. These weren\'t the beasts you were looking for, but they do carry a lot of disguises that your employer would probably be most interested in seeing. One of the men wonders aloud.%SPEECH_ON%So what were they playing dress up for?%SPEECH_OFF%%bro2% folds some of the disguises over his arm as he goes about collecting them.%SPEECH_ON%Suicide by ceremony? Their dance and fun got our attention, after all.%SPEECH_OFF%He picks up one of the disguises only for the head of the dead to get slinged up with it. The warrior laughs as he kicks the dead man\'s head out.}",
			Image = "",
			List = [],
			Options = [
				{
		...
		this.m.Screens.push({
			ID = "Success4",
			Title = "On your return...",
			Text = "[img]gfx/ui/events/event_04.png[/img]{You enter %employer%\'s office carrying the dead spider on your back. The man screams and his chair squalls as it flings back across the floor. He jumps to his feet and draws a knife off his desk. You throw the dead webknecht from your shoulder and it clatters on its back. The chieftain slowly comes forward. He sheathes the knife in a loaf of bread and shakes his head.%SPEECH_ON%By the old gods, you nearly gave me a heart attack.%SPEECH_OFF%Nodding, you tell the man it required more than a big boot to squash these beasts. He nods back.%SPEECH_ON%Of course, warrior, of course! Your payment of %reward_completion% crowns is right there in the corner. And, please, take that beastly thing with you when you leave.%SPEECH_OFF% | Cats hiss and flee the second you step into %employer%\'s room. A few dogs, always the sort for a mystery, run about your legs and sniff the spider carcass, their noses crinkling and pulling away but always coming back for more. The chieftain looks as he can hardly believe his eyes. %SPEECH_ON%Is that a giant spider?%SPEECH_OFF%You nod. He just smiles and replies.%SPEECH_ON%Perhaps I should have suggested you bring a very big shoe. Your payment of %reward_completion% crowns is there in the satchel. Go on, take it. It\'s all there. And you can leave the corpse. I\'d like to get a closer look at the creature.%SPEECH_OFF% | %employer% is hosting some clansmen when you enter his room with a giant dead spider and fling the corpse across the floor. Its bristly hairs hiss as they scratch across the stone and its eight legs scuttle upside down like some furniture of horror and it strays sideways and pops off the corner of a bookshelf and flips onto its toes and prongs there as though ready to pounce. Chaos breaks out as everyone screams and runs out the door or bails from the nearest open window, a litter of colorful confetti playfully twirling in their wake. The chieftain stands alone amongst the emptied space and purses his lips.%SPEECH_ON%Truly, warrior, was that necessary?%SPEECH_OFF%You nod and tell him that hiring you was necessary and that paying you will still be very necessary. The man nods his head and gestures to the corner of the room.%SPEECH_ON%Your satchel\'s over there with %reward_completion% crowns, as agreed upon. Now get that awful thing out of here and tell those fine folks they have a reason to celebrate.%SPEECH_OFF%}",
			Image = "",
			List = [],
			ShowEmployer = true,
			Options = [
