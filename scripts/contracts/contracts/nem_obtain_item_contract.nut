this.nem_obtain_item_contract <- this.inherit("scripts/contracts/barbarian_contract", {
	...
	function create()
	{
		...
		this.m.Type = "contract.nem_obtain_item";
	...
	function createScreens()
	{
		...
		this.m.Screens.push({
			ID = "Task",
			Title = "Negotiations",
			Text = "{[img]gfx/ui/events/event_62.png[/img]You find %employer% talking to a druid cloaked in shapes more familiar to beasts than man. Horns for a helmet, bearskin for armor, and the hooves of deer clattering around his chest in a brutish necklace. He\'s quite the sight. Seeing you, %employer% waves you in.%SPEECH_ON%Warrior! It is good to see you--%SPEECH_OFF%The druid pushes the man out of the way mid-talk. He speaks with a wobble in his voice, as though he were speaking from the depths of a cavern.%SPEECH_ON%A fighter, ha! Surely you are a man of faith, no? We of %townname% have lost the %item%. This relic is of great import to us, for through it we can speak to the old gods and have our prayers answered. It\'s been stolen away, in some manner or another, to the %location%. Go there and retrieve it.%SPEECH_OFF%You glance at %employer% who nods.%SPEECH_ON%Aye, what he said.%SPEECH_OFF%}",
			Image = "",
			List = [],
			ShowEmployer = true,
			ShowDifficulty = true,
		...
		this.m.Screens.push({
			...
			Text = "[img]gfx/ui/events/event_04.png[/img]{You find %employer% sitting in the center of the settlement, his arms up to the skies, his eyes closed, his mouth murmuring prayers. The clanfolk are all around him, kneeling and doing the same. You pick up a rock and hurl at a weathervane, the clank and rustic spins drawing everyone\'s attention.\n\nYou hold the relic up so all can see. %employer_short% jumps to his feet and takes the %item%. The people roar with delight, speaking of good things to come. Your payment is handed to you which, truly, is what you would consider a \'good thing\'.}",
			Image = "",
			Characters = [],
			List = [],
			ShowEmployer = true,
			Options = [
				{
					Text = "The clansfolk seem to be in a good spirit now.",
					function getResult()
					{
		...
		this.m.Screens.push({
			ID = "Failure1",
			Title = "On your return...",
			Text = "[img]gfx/ui/events/event_43.png[/img]{The clansfolk of %townname% are eagerly awaiting your return. A shame, because you don\'t have the relic that they so desperately need. %employer%, seeing your failure a step ahead of the clansmen, meets you at the camp entrance and talks to you in whispers.%SPEECH_ON%I take it you do not have the %item%.%SPEECH_OFF%You try to explain all that happened, but he does not seem to listen.%SPEECH_ON%It\'s no matter, warrior. I can\'t pay you, obviously, and the clansfolk shan\'t hear of your shortcomings lest we have them lose their minds. They depend upon idols to find comfort in this world. I will have to come up with my own solution and, well, pray it works. Good day.%SPEECH_OFF% | You return to %employer% emptyhanded. He takes you by the side and whispers.%SPEECH_ON%And why have you come at all? Do you not understand what import these clansfolk have put upon the idol? Without it to worship, they\'ll have nothing to believe in. Men of strong faith needs somewhere to put it. If he can\'t find it, all he finds is himself. And, like an ugly brute staring into a mirror, we needn\'t rush to see the anger and confusion in the reflection of the idol\'s absence. Leave, warrior, before the people see you\'ve not returned with the %item%.%SPEECH_OFF%}",
			Image = "",
			Characters = [],
			List = [],
			ShowEmployer = false,
